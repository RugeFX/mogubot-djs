...
The migration service runs `npm run db:migrate` on startup and the bot waits for it to finish successfully. No character catalog is bundled; see `data/README.md` before using `/wish`. To seed a local `data/characters.json` after startup, run `docker compose run --rm -v "$(pwd)/data:/app/data:ro" migrate npm run db:seed`.
